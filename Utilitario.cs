...
namespace ComissPro
{
    public static class Utilitario
    {
        ...
        public static int GerarProximoCodigo(string query)
        {
            ...
            using (SQLiteConnection conn = Conexao.Conex()) // Inicializando a conexão aqui
            {
                try
                {
                    conn.Open();
                ...
                {
                    // Tratar exceção de SQLite
                    Console.WriteLine($"Erro de SQLite: {SQLiteEx.Message}");
                }
        ...
        public static void PreencheComboBoxFacil(KryptonComboBox comboBox, string query, string nome, string Id)
        {
            using (SQLiteCommand cmd = new SQLiteCommand(query, conn))
            {
                conn.Open();
        ...
        public static void PesquisarPorNomeMensagemSuprimida(string query, string nomeParametro, string valorParametro, KryptonDataGridView dgv)
        {
            try
            {
                using (SQLiteConnection conn = Conexao.Conex())
                {
                    conn.Open();
                    using (SQLiteCommand cmd = new SQLiteCommand(query, conn))
                    {
...
private void FrmMeuFormulario_Load(object sender, EventArgs e)
    ...
    private void textBox_TextChanged(object sender, EventArgs e)
    {
        ...
        public static void LocalizarGeral(DataGridView dataGridView, string query)
        {
            using (conn)
            {
                using (SQLiteCommand comando = new SQLiteCommand(query, conn))
                {
                    try
                    {
                        conn.Open();
                    ...
                    {
                        MessageBox.Show($"Erro de SQLite: {SQLiteEx.Message}", "Erro de SQLite", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                    catch (InvalidOperationException invOpEx)
                    {
        ...
        public static bool VerificarDuplicidade(string query, Dictionary<string, object> parametros)
        {
            using (conn)
            {
                using (SQLiteCommand comando = new SQLiteCommand(query, conn))
                {
                    ...
                    {
                        // Trate a exceção de SQLite conforme necessário
                        Console.WriteLine($"Erro de SQLite: {SQLiteEx.Message}");
                        return false;
                    }
                    catch (InvalidOperationException invOpEx)
        ...
        */
        ...
        public static void PesquisarProdutoPorReferencia(string referencia, KryptonTextBox txtNomeProduto, KryptonTextBox txtPrecoProduto)
        {
            ...
            using (var conn = Conexao.Conex())
            {
                ...
                {
                    DataTable dt = new DataTable();
                    SQLiteCommand comando = new SQLiteCommand("SELECT NomeProduto, PrecoDeVenda FROM Produtos WHERE Referencia = @Referencia", conn);
        ...
        public static void PesquisarProdutoPorReferencia2(string referencia, KryptonTextBox texboxReferencia, KryptonTextBox texboxNome, KryptonTextBox texboxProdutoID)
        {
            ...
            try
            {
                using (SQLiteConnection conexao = Conexao.Conex())
                {
                    conexao.Open();
